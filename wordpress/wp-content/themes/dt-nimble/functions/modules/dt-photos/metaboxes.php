...
function dt_metabox_photos_layout_options( $post ) {
	...
	echo '<p><strong>' . _x( 'Thumbnail height', 'backend photos layout', LANGUAGE_ZONE ) . '</strong></p>';
	echo dt_melement( 'text', array(
		'name'			=> $box_name . '_thumb_height',
		'description'	=> _x('(in pixels). If not specified, default value will be taken.', 'backend photos layout', LANGUAGE_ZONE),
...
function dt_metabox_photos_layout_options_save( $post_id ) {
	...
	if( isset($_POST[$box_name.'_thumb_height']) ) {
		$mydata['thumb_height'] = empty( $_POST[$box_name.'_thumb_height'] ) ? '' : intval( $_POST[$box_name.'_thumb_height'] );
	}
