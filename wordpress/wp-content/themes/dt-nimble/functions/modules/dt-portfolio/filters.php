...
function dt_f_portfolio_mu( $tabs ) {
	if ( isset( $_REQUEST['post_id'] ) && 'dt_portfolio' == get_post_type( $_REQUEST['post_id'] ) && ! empty( $_GET['dt_custom'] ) ) {
		global $wpdb;
		...
		if( !isset($tabs['dt_portfolio_media'])) {
			$tabs['dt_portfolio_media'] = sprintf( _x( 'Slides (%s)', 'backend portfolio', LANGUAGE_ZONE ), "<span id='attachments-count'>$attachments</span>");
		}
...
function dt_f_portfolio_att_fields($fields, $post) {
	if( 'dt_portfolio' == get_post_type($post->post_parent) ) {
		...
		// if its our upload metabox - remove unnecessary fields
...
function dt_f_portfolio_att_fields_save( $post, $attachment ) {
    ...
    if( 'dt_portfolio' == get_post_type($_REQUEST['post_id']) && !empty($_GET['dt_custom']) ) {
...
function dt_f_portfolio_col_thumb( $defaults ){
    $defaults['dt_portfolio_cat'] = _x( 'Category', 'backend portfolio', LANGUAGE_ZONE );
