...
function dt_f_slider_mu( $tabs ) {
	if ( isset( $_REQUEST['post_id'] ) && 'dt_slider' == get_post_type( $_GET['post_id'] ) && ! empty( $_GET['dt_custom'] ) ) {
		global $wpdb;
		...
		if( !isset($tabs['dt_slider_media'] )) {
			$tabs['dt_slider_media'] = sprintf( _x( 'Slides (%s)', 'backend slideshow', LANGUAGE_ZONE ), "<span id='attachments-count'>$attachments</span>");
		}
...
function dt_f_slider_att_fields_save($post, $attachment) {
    ...
    if( 'dt_slider' == get_post_type($_REQUEST['post_id']) && !empty($_GET['dt_custom']) ) {
...
function dt_f_slider_col_thumb( $defaults ){
