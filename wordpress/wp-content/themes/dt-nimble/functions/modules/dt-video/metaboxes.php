...
function dt_metabox_video_layout_options( $post ) {
	...
	echo '<p><strong>' . _x( 'Thumbnail height', 'backend video layout', LANGUAGE_ZONE ) . '</strong></p>';
	echo dt_melement( 'text', array(
		'name'			=> $box_name . '_thumb_height',
		'description'	=> _x('(in pixels). If not specified, default value will be taken.', 'backend video layout', LANGUAGE_ZONE),
	...
	echo '<div class="dt_hr"></div>';
	echo '<p class="dt_switcher-box"><strong>' . _x('Show Category(s)', 'backend portfolio layout', LANGUAGE_ZONE) . '</strong>';
	dt_core_mb_draw_radio_switcher( $box_name . '_show_category', $opts['show_category'], $radio_on_off );
	echo '</p>';
...
function dt_metabox_video_layout_options_save( $post_id ) {
	...
	if( isset($_POST[$box_name.'_thumb_height']) ) {
		$mydata['thumb_height'] = '' == $_POST[$box_name.'_thumb_height'] ? '' : intval( $_POST[$box_name.'_thumb_height'] );
	}
	...
	if( isset($_POST[$box_name.'_show_category']) ) {
		$mydata['show_category'] = esc_attr( $_POST[$box_name.'_show_category'] );
	}
...
function dt_metabox_video_options( $post ) {
    ...
    $link = dt_melement( 'text', array(
        ...
        'class'         => 'widefat',
        'description'   => _x( 'Video link:', 'backend video', LANGUAGE_ZONE ),
        'wrap'          => '%2$s%1$s',
