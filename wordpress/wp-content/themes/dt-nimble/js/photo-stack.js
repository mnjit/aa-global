/*Photo Stack Gallery*/
     jQuery(function($) {
				...
				*/
				...
				$('#ps-slider').touchwipe({
					wipeLeft: function() { $('.navig-nivo').find('.next').trigger("click"); },
					wipeRight: function() { $('.navig-nivo').find('.prev').trigger("click"); },
					min_move_x: 30,
					min_move_y: 20,
				...
				if(elems > 4) {
					...
					if (autoslideOn == "1") {
						setInterval(function() {
							$(".navig-nivo.ps a.next:not(.temp_disabled)").trigger("click")
						}, autoslideInterval)
					}
