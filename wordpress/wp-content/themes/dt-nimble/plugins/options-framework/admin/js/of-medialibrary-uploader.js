...
/*-----------------------------------------------------------------------------------*/
    ...
    removeFile: function () {
     ...
     $('.mlu_remove').live('click', function(event) { 
		...
		var id = $(this).parents().parents().children('.upload-id');
		if( id ) { $(this).parents().parents().children('.upload-id').attr('value', 0); }
...
/*-----------------------------------------------------------------------------------*/
	...
	var formfield,
        ...
        formfield = $(this).prevAll('input.upload').attr('id');
	...
	window.send_to_editor = function(html) {
		if (formfield) {
		  ...
		  if ( $(html).html(html).find('img').length > 0 ) {
			...
			// get attachment id
			itemid = $( html ).html(html).find( 'img' ).attr( 'class' ).split( 'wp-image-' );
			if ( itemid[1] ) {
				itemid = itemid[1].split(' ')[0];
			} else {
				itemid = 0;
			}
		  ...
		  // if input for attacment id exists - fill it with id
		  if( $('#' + formfield).next('input.upload-id') ) {
			$('#' + formfield).next('input.upload-id').val( itemid );
		  }
