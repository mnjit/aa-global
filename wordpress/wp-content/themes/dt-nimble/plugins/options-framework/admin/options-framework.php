...
if ( ! function_exists( 'optionsframework_add_page' ) ) {
    function optionsframework_add_page() {
        ...
        $main_page = add_menu_page( 'Skins', 'Theme Options', 'edit_theme_options', 'options-framework', 'optionsframework_page' );
        ...
        if ( isset( $submenu['options-framework'] ) ) {
            $submenu['options-framework'][0][0] = 'Skins';
        }
...
if ( !function_exists( 'optionsframework_page' ) ) {
	function optionsframework_page() {
		...
		if ( isset( $_GET['settings-updated'] ) && 'true' == $_GET['settings-updated'] && of_get_option( 'misc-static_css', false ) && optionsframework_create_stylesheet() ) {
			return;
		}
				...
				<div id="optionsframework" class="postbox">
					<form action="options.php" method="post">
						...
						<div id="optionsframework-submit">
							<input type="submit" class="button-primary" name="update" value="<?php echo esc_attr( _x( 'Save Options', 'optionsframework', LANGUAGE_ZONE ) ); ?>" />
							<input type="submit" class="reset-button button-secondary" name="reset" value="<?php echo esc_attr( _x( 'Restore Defaults', 'optionsframework', LANGUAGE_ZONE ) ); ?>" onclick="return confirm( '<?php print esc_js( _x( 'Click OK to reset. Any theme settings will be lost!', 'optionsframework', LANGUAGE_ZONE ) ); ?>' );" />
							<div class="clear"></div>
						</div>
					</form>
				</div> <!-- / #container -->
...
function optionsframework_validate( $input ) {
	...
	if ( isset( $clean['of-preset'] ) ) {
		$preserve = array(
			...
			'misc-contact_email',
			'misc-contact_skype',
			'misc-contact_work_hours',
			'misc-show_next_prev_post',
			'misc-show_next_prev_portfolio',
			'misc-show_top_line',
			'misc-show_search_top',
			'misc-show_author_details',
			'misc-analitics_code',
			'menu-parent_menu_clickable',
			'social-like_post',
			'social-like_portfolio',
			'social-like_catalog',
			'social-like_gallery',
			'social-like_photo',
			'social-like_video',
			'social-like_button_google_plus',
			'social-like_button_twitter',
...
function optionsframework_adminbar() {
	...
	$wp_admin_bar->add_menu( array(
		'parent'    => 'appearance',
		'id'        => 'of_theme_options',
		'title'     => _x( 'Theme options', 'optionsframework', LANGUAGE_ZONE ),
