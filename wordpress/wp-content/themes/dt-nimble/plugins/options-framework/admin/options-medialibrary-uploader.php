...
if ( ! function_exists( 'optionsframework_mlu_init' ) ) {
	function optionsframework_mlu_init () {
		register_post_type( 'optionsframework', array(
			'labels' => array(
				'name' => _x( 'Options Framework Internal Container', 'backend', LANGUAGE_ZONE ),
			),
			'public' => true,
			'show_ui' => false,
...
if ( ! function_exists( 'optionsframework_mlu_modify_tabs' ) ) {
	...
	function optionsframework_mlu_modify_tabs ( $tabs ) {
		$tabs['gallery'] = str_replace( _x( 'Gallery', 'backend', LANGUAGE_ZONE ), _x( 'Previously Uploaded', 'backend', LANGUAGE_ZONE ), $tabs['gallery'] );
		return $tabs;
	}
}
