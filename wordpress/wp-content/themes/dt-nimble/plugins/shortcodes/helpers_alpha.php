...
function dt_admin_order_options( $opts = array(), $echo = true ) {
	...
	$orderby_white_list = array( 'date', 'ID', 'author', 'name', 'title', 'modified', 'rand', 'menu_order' );
    if ( !empty($options['orderby_white_list']) && is_array($options['orderby_white_list']) ) {
        $orderby_white_list = $options['orderby_white_list'];
    }

    $output = '';
    
    $output .= $before_element . dt_admin_page_option_orderby( $box_name, $options['orderby'], $orderby_white_list ) . $after_element;
...
// orderby
function dt_admin_page_option_orderby( $box_name, $current = false, $white_list = array() ) {
    if( empty($box_name) ) { return false; }
    
    $p_orderby = array(
        'date'              => __( 'Order by date', LANGUAGE_ZONE ),
        'ID'                => __( 'Order by ID', LANGUAGE_ZONE ),
        'author'            => __( 'Order by author', LANGUAGE_ZONE ),
        'name'              => __( 'Order by name', LANGUAGE_ZONE ),
        ...
        'parent menu_order' => __( 'Order by parent', LANGUAGE_ZONE ),
    );
	
    if ( !empty($white_list) ) {
        foreach ( array_diff( array_keys($p_orderby), $white_list ) as $key ) {
            unset($p_orderby[ $key ]);
        }
    }
