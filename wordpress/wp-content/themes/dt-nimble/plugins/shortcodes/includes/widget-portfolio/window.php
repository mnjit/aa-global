...
<head>
    ...
    function init() {
        ...
        jQuery.post(
            ...
            function( response ){
				...
				if( (temp2 = temp.match(/(thumb_height)=["|']{1}(.*?)["|']{1}/i)) != null ) {
					var val = ( isNaN ( temp2[2] ) ) ? '' : Math.abs ( temp2[2] );
                    jQuery('#dt_thumb_height').val( val );
                } 
	...
	function insertShortcode() {
        ...
        var thumb_height = jQuery('#dt_thumb_height').val();
		if ( thumb_height.length > 0 ) {
			thumb_height = ' thumb_height="' + parseInt( thumb_height ) + '"';
		}
        ...
        tagtext = '[dt_portfolio title="'+title+'" ppp="'+ppp+'" orderby="'+orderBy+'" order="'+order+'" column="'+column+'" lines="'+lines+'" autoslide="'+autoslide+'" showdesc="'+showdesc+'"'+thumb_height+filter+'/]';	
